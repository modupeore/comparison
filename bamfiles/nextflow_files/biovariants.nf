...
process idxstats {
  ...
  input:
    set val(prefix), file (bam), file(bai) from grouped1

  output:
    file '*' into idxstatsOut
...
process sortsam {
  ...
  input:
    set val(prefix), file (bam), file(bai) from grouped2 
